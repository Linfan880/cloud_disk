...
int pv_file(char *md5, char *filename)
{
    ...
    //sql语句
    //查看该共享文件的pv字段
    sprintf(sql_cmd, "select pv from share_file_list where md5 = '%s' and filename = '%s'",
            md5, filename);
    ...
    //更新该文件pv字段，+1
    sprintf(sql_cmd, "update share_file_list set pv = %d where md5 = '%s' and filename = '%s'",
            pv+1, md5, filename);
...
int cancel_share_file(char *user, char *md5, char *filename)
{
    ...
    //===1、mysql记录操作
    //sql语句
    sprintf(sql_cmd, "update user_file_list set shared_status = 0 where user = '%s' and md5 = '%s' and filename = '%s'",
            user, md5, filename);
